...
<!-- ########################################################################################## -->
    ...
    <div class="MainContainer">
        ...
        $rackquery = mysqli_query($conn, "SELECT * FROM Racks");
        ...
        <table id="customers">
            ...
            <tr>
                <td style="text-align: center;"><?php echo $row["Rack_ID"]; ?></td>
                <td style="text-align: center;"><?php echo $row["Tub_Count"]; ?></td>
                <td style="text-align: center;"><?php echo $row["Tub_Size"]; ?>L</td>
                <td><form action="/Reptiserver/Functions/delete_rack.php" method="post"><input type="hidden" id="rack" name="rack" value="<?php echo $row["Rack_ID"]; ?>"><input type="hidden" id="source" name="source" value="/Reptiserver/Functions/view_racks.php"><button onclick="return clicked();" type="submit" class="btn" style="background-color:darkred;margin-top:22px;"><i class="fa fa-trash" aria-hidden="true"></i></button></form></td>
            </tr>
            <?php } ?>
            </tbody>
        </table>
        ...
        <button onclick="document.getElementById('rack_create_modal').style.display='block'" style="width:40%;margin-left: 30%;margin-right:30%" id="btn1"><b>New Rack</b></button>
        </div>
    </div>
...
function clicked(e)
{
    if(!confirm('Are you sure you wish to delete this rack?')) {
        e.preventDefault();
    }
}
